...
class PflugervilleLibraryScraper(CalendarScraper):
    ...
    def __init__(self, calendar_id=None, category_calendar_ids=None):
        ...
        self.category_ids = {  # Adding the category Ids for ease of use in my script.
            "Library": "129e1957-debe-4e2e-9acb-068c54de7c71",  # TODO: Check on these
    ...
    def _get_token_from_html(url):
        ...
        if url != "https://www.pflugervilletx.gov/372/Library-Event-Calendar":
            raise ValueError("Invalid URL provided. Expected the library event calendar URL.")
        try:
    ...
    def _get_events(
        ...
        if start_date and end_date:
            start_date_str = start_date.strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "Z"  # Limit to milliseconds
            end_date_str = end_date.strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "Z"  # Limit to milliseconds
            filter_parts.append(
